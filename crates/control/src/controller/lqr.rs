use crate::{ControlEffort, ControlState, Controller};
...
impl Controller for LqrController {
    ...
    fn compute(&mut self, state: &ControlState) -> Result<ControlEffort, Self::Error> {
        ...
        let raw = -feedback;
        let bounded = raw.max(-self.max_abs_effort).min(self.max_abs_effort);
        ControlEffort::try_new(bounded).map_err(|_| LqrError::Numeric)
    }
}
...
mod tests {
    ...
    fn lqr_uses_theta_theta_dot_phi_phi_dot_order() {
        let mut controller = LqrController::new([2.0, 3.0, 5.0, 7.0], 0.5).unwrap();
        let state = ControlState::new(1.0, 2.0, 3.0, 4.0, 10);
        assert_eq!(controller.compute(&state).unwrap().value(), -0.5);
    }
}
